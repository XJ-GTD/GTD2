...
//日程标签表-备忘录标签(里程碑、随手记、记账)
export class RcbfvEntity {
  ...
  private _dt:string=''; //日期（重复日期开始时间）
  private _wd:string = ''; //完成时间
  private _rm:string=''; //备注
  private _ac:string=''; //闹铃提醒类型
  ...
  private _dsq:string;
  //查询单个
  private _qosq:string = 'select * from GTD_C_MO where sI="' + this._sI+'"';
  ...
  get dsq(): string {
    let sql='DELETE FROM GTD_C_MO WHERE 1=1 ';
    if(this._sI!=null){

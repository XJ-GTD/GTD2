...
import {Config, Form, Item, Picker, PickerColumn, PickerController} from "ionic-angular";
import {
  compareDates,
  convertDataToISO,
...
import {assert, clamp, isArray, isBlank, isObject, isPresent, isString} from "ionic-angular/util/util";
...
export class DatePickerComponent extends BaseInput<DateTimeData> implements AfterContentInit, ControlValueAccessor, OnDestroy {
  ...
  generate() {
    ...
    if (isPresent(template)) {
      ...
      parseTemplate2(template).forEach(format => {
        ...
        if (isPresent((<any>this)[key + 'Values'])) {
          ...
          values = dateValueRange2(format, this._min, this._max);
        }
        ...
        const column: PickerColumn = {
          ...
          options: values.map(val => {
            return {
              value: val,
              text: renderTextFormat2(format, val, null, this._locale),
            };
          })
        };

...
//日程标签表-纪念日标签（特殊日期,法定假日）
export class RcbfEntity {
  ...
  private _ac:string=null; //是否需要闹铃0否1是
   ...
   * @private
   */
  private _csq:string = 'CREATE TABLE IF NOT EXISTS GTD_C_JN(sI VARCHAR(100) PRIMARY KEY,' +
  ...
  private _drsq:string="DROP TABLE IF EXISTS GTD_C_JN;"

  private _isq:string;
  private _usq:string;
  private _dsq:string;
  //查询单个
  private _qosq:string = 'select * from GTD_C_JN where sI=' + this._sI;
  ...
  get dsq(): string {
    let sql='DELETE FROM GTD_C_JN WHERE 1=1 ';
    if(this._sI!=null){

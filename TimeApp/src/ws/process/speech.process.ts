  ...
  constructor(private assistant: AssistantService,
   ...
   gowhen(content: WsContent, contextRetMap: Map<string,any>) {
    ...
    return new Promise<Map<string,any>>(async resolve => {
      ...
      //处理所需要参数
      let serverratio = content.thisContext.context.client? content.thisContext.context.client.serverratio : 0;

      let ratios = "";

      if (content.thisContext.context.client && content.thisContext.context.client.ratios && content.thisContext.context.client.ratios.length > 1) {

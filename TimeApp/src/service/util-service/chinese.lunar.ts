...
export class ChineseLunar {
  ...
  /*
     * 分析日期表达式，并提取其中的单位和数值
     */
  expression(expr):Array<any> {
    let list = expr.match(/[+-]?\d+((ms)|[yMdhmsw])/g);
    let result = [];
  ...
  /*
     ...
     */
  findLunar(solar, index, minMonth, maxMonth, isPreYear) {
    //取得映射的数据
    let mapping = this.MAPPING[index];
  ...
  lunarToSolar(year, month, day, leap) {
    ...
    //取得润月是哪一个月
    let leapMonth = this.MAPPING[offset][0];
    ...
    //取出对应到某个月的片断
    let segment:any = this.MAPPING[offset][month]; 		//农历第一天对应公历的具体天
    let mapMonth = Number(segment.substring(0, 2))
    let mapDate = Number(segment.substring(2, 4));
    ...
    let solar = new Date(year, mapMonth - 1, mapDate);
    let time = solar.getTime() + ((day - 1) * 24 * 60 * 60 * 1000);
  ...
  /*
     ...
     3.计入偏移量
     */
  solarToLunar(solar, format):string {
    let offset = solar.getFullYear() - this.MINYEAR;
    //超出范围
    if (offset <= 0 || offset >= this.MAPPING.length) {
      throw new Error('Specified date range is invalid.');
    }
    ...
    //查找范围内的农历数据
    let data:any = this.findLunar(solar, offset, 0, 13, false);
  ...
  traditionalYearName(year) {
    let Gan = "甲乙丙丁戊己庚辛壬癸".split("");
    let Zhi = "子丑寅卯辰巳午未申酉戌亥".split("");
  ...
  //获取中文的年
  yearName(year) {
    let cnStr = '〇,一,二,三,四,五,六,七,八,九'.split(",");
  ...
  monthName(month, traditional, leap) {
    let monthName = "正,二,三,四,五,六,七,八,九,十,十一,十二".split(",");
    if (traditional) {
      monthName[11] = "腊"
    }
  ...
  //计算公历两个日期之差
   private solarDiff(left:any, right:any, interval):number {
    let span:any = left.getTime() - right.getTime(); //相差毫秒
    switch (interval) {
  ...
  //格式化农历日期，date是农历的日期
  format(lunar, expr) {
    return expr.replace(/[TAYyMmdD]/g,(m, i)=> {
      switch (m) {
        //获取传统的年
        case "T":
          return this.traditionalYearName(lunar.year);
        //获取生肖
        case "A":
          return this.animalName(lunar.year);
        //获取中文的年
        case "Y":
          return this.yearName(lunar.year);
        //获取数字年
        case "y":
          return lunar.year;
        //获取月份
        case "m":
          return this.monthName(lunar.month, false, lunar.leap);
        //获取传统的月
        case "M":
          return this.monthName(lunar.month, true, lunar.leap);
        //获取天
        case "d":
          return this.dayName(lunar.day);
        //如果是初一，则显示月，而不是显示
        case "D":
          if (lunar.day == 1) {
            return this.monthName(lunar.month, false, lunar.leap);

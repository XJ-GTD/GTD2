...
export class MemberService {
   ...
   * @returns {Promise<Array<FsData>>}
   */
  shareMember(si: string, memberList: Array<Member>): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      ...
      for (let member of memberList) {
        if(member.rc != UserConfig.account.phone){
      ...
      this.agdRest.contactssave(adgPro).then(data => {
        ...
        for (let member of memberList) {
          ...
          for (let ofs of data) {
            if (ofs.pwi == member.pwi) {
              is = true;
              break;
            }
          }
          if (!is && member.rc != UserConfig.account.phone) {
            let dt = new DTbl();

function shouldclean(datasource) 
{
  ...
  for (var di in input.data) {
	var data = input.data[di];
  	if (data['sub'] === 'nlp' && data['intent']['service'] === 'OS6981162467.LendMoney' && data['intent']['intentType'] === 'custom' && data['intent']['semantic']) {
...
function clean(datasource) 
{
  ...
  for (var di in input.data) {
    ...
    if (dt['sub'] === 'nlp' && dt['intent']['service'] === 'OS6981162467.LendMoney' && dt['intent']['intentType'] === 'custom' && dt['intent']['semantic']) {
      data = dt;
    }
  }
  ...
  standardnext['function'] = 'lend';
  standardnext['data'] = {when: sdate, who: who, money: money};

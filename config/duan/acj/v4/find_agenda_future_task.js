    ...
    for (var di in input.data) {
      ...
      if (data['sub'] === 'nlp' && data['intent']['service'] === 'OS6981162467.FindAgenda' && data['intent']['intentType'] === 'custom' && data['intent']['semantic']) {
          ...
          for (var sei in semantics) {
            ...
            if (semantic['intent'] === 'FindByFutureTask') {
              return true;
            }
          }
      }
...
function clean(datasource)
{
  ...
  output.content['2'] = {
    when: 'function(agendas, showagendas, contacts, branchtype, branchcode) { if (isFuturefulltime(agendas)) { return true; } else { return false; }}',
    processor: 'S',
    option: 'S.P',
    parameters: {
      t: 'FTO'
    ...
    input: {
      textvariables: [
        ...
        {name: 'startTime', expression: 'agendas[0].st', default: '全天'},
        {name: 'endTime', expression: 'agendas[1].et', default: '全天'}
      ]
    }
  };

  output.content['3'] = {
    when: 'function(agendas, showagendas, contacts, branchtype, branchcode) { if (isFutureparttime(agendas)) { return true; } else { return false; }}',
    processor: 'S',
    option: 'S.P',
    parameters: {
      t: 'FTT'

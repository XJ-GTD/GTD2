...
function shouldclean(datasource)
{
  ...
  if (input.data && input.data[0] !== undefined) {
    for (var di in input.data) {
      ...
      if (data['sub'] === 'nlp' && data['intent']['service'] === 'OS6981162467.UseHelper' && data['intent']['intentType'] === 'custom' && data['intent']['semantic']) {
          var semantics = data['intent']['semantic'];

          for (var sei in semantics) {
            var semantic = semantics[sei];

            if (semantic['intent'] === 'OthersUnstandard' || semantic['intent'] === 'HowtoCancel' || semantic['intent'] === 'HowtoModifyAgenda' || semantic['intent'] === 'HowtoSearch' || semantic['intent'] === 'HowtoUse' || semantic['intent'] === 'HowtoCreateAgenda' || semantic['intent'] === 'HowToUseMWXing') {
              return true;
            }
          }
      }
...
function clean(datasource)
{
  ...
  for (var di in input.data) {
    ...
    if (dt['sub'] === 'nlp' && dt['intent']['service'] === 'OS6981162467.UseHelper' && dt['intent']['intentType'] === 'custom' && dt['intent']['semantic']) {
      data = dt;
    }
  }
  var output = {};

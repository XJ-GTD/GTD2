...
if [[ $TRAVIS_OS_NAME == 'osx' ]]; then
  ...
  ls $TRAVIS_BUILD_DIR/build/debug/$IOS_APP_NAME.xcarchive/dSYMs
  xcodebuild -exportArchive -archivePath $TRAVIS_BUILD_DIR/build/debug/$IOS_APP_NAME.xcarchive -exportPath $TRAVIS_BUILD_DIR/build/debug/$IOS_APP_NAME.ipa -exportOptionsPlist $TRAVIS_BUILD_DIR/travis/profiles/ios/exportOptions.plist
else

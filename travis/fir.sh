  ...
  if [ $TRAVIS_JOB_NAME = 'unittest' ]; then
    ...
    if [ -f "$TRAVIS_BUILD_DIR/TimeApp/platforms/android/app/build/outputs/apk/debug/app-debug.apk" ]; then
      ...
      if [ $TRAVIS_BRANCH = 'cassiscornuta' ]; then
        # --switch_to_qiniu
        # --oversea_turbo
        fir p $TRAVIS_BUILD_DIR/TimeApp/platforms/android/app/build/outputs/apk/debug/app-$TRAVIS_BRANCH.apk -T $FIR_TOKEN_CASSISCORNUTA -c "唐冠螺 (Android): ${TRAVIS_COMMIT_MESSAGE}" --oversea_turbo
      else

...
if [ $TRAVIS_OS_NAME = 'osx' ]; then
    ...
    if [ $TRAVIS_BRANCH = 'cassiscornuta' ]; then
      fir publish $TRAVIS_BUILD_DIR/build/debug/app-$TRAVIS_BRANCH.ipa -T $FIR_TOKEN_CASSISCORNUTA -c "#${TRAVIS_BRANCH}: ${TRAVIS_COMMIT_MESSAGE}"
    else
    ...
    if [ $TRAVIS_BRANCH = 'cassiscornuta' ]; then
      fir p $TRAVIS_BUILD_DIR/TimeApp/platforms/android/app/build/outputs/apk/debug/app-$TRAVIS_BRANCH.apk -T $FIR_TOKEN_CASSISCORNUTA -c "#${TRAVIS_BRANCH}: ${TRAVIS_COMMIT_MESSAGE}"
    else

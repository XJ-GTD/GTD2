...
public class Pinyin4j {
    private static Pinyin4j pinyin4j = new Pinyin4j();

    public static String toPinYin(String hanyu){
        if (pinyin4j == null) pinyin4j = new Pinyin4j();

        try {
            return pinyin4j.toPinYin(hanyu, "", Type.LOWERCASE);
...
//                    }
     ...
     * @throws BadHanyuPinyinOutputFormatCombination 异常信息
     */
    public String toPinYinFirst(String str, String spera, Type type) throws BadHanyuPinyinOutputFormatCombination {
        if(str == null || str.trim().length()==0) {
            return "";
        }
        if(type == Type.UPPERCASE) {

package com.xiaoji.gtd.dto.sync;

/**
 * 计划表数据
 *
 * create by wzy on 2018/12/28
 */
public class SyncTableData {

    private String tableA;
    private String tableB;
    private String tableC;
    private String tableD;
    private String tableE;
    private String tableF;
    private String tableG;
    private String tableH;
    private String tableI;
    private String tableJ;
    private String tableK;
    private String tableL;
    private String tableM;
    private String tableN;
    private String tableO;
    private String tableP;
    private String tableQ;
    private String tableR;
    private String tableS;
    private String tableT;
    private String tableU;
    private String tableV;
    private String tableW;
    private String tableX;
    private String tableY;
    private String tableZ;

    public String getTableA() {
        return tableA;
    }

    public void setTableA(String tableA) {
        this.tableA = tableA;
    }

    public String getTableB() {
        return tableB;
    }

    public void setTableB(String tableB) {
        this.tableB = tableB;
    }

    public String getTableC() {
        return tableC;
    }

    public void setTableC(String tableC) {
        this.tableC = tableC;
    }

    public String getTableD() {
        return tableD;
    }

    public void setTableD(String tableD) {
        this.tableD = tableD;
    }

    public String getTableE() {
        return tableE;
    }

    public void setTableE(String tableE) {
        this.tableE = tableE;
    }

    public String getTableF() {
        return tableF;
    }

    public void setTableF(String tableF) {
        this.tableF = tableF;
    }

    public String getTableG() {
        return tableG;
    }

    public void setTableG(String tableG) {
        this.tableG = tableG;
    }

    public String getTableH() {
        return tableH;
    }

    public void setTableH(String tableH) {
        this.tableH = tableH;
    }

    public String getTableI() {
        return tableI;
    }

    public void setTableI(String tableI) {
        this.tableI = tableI;
    }

    public String getTableJ() {
        return tableJ;
    }

    public void setTableJ(String tableJ) {
        this.tableJ = tableJ;
    }

    public String getTableK() {
        return tableK;
    }

    public void setTableK(String tableK) {
        this.tableK = tableK;
    }

    public String getTableL() {
        return tableL;
    }

    public void setTableL(String tableL) {
        this.tableL = tableL;
    }

    public String getTableM() {
        return tableM;
    }

    public void setTableM(String tableM) {
        this.tableM = tableM;
    }

    public String getTableN() {
        return tableN;
    }

    public void setTableN(String tableN) {
        this.tableN = tableN;
    }

    public String getTableO() {
        return tableO;
    }

    public void setTableO(String tableO) {
        this.tableO = tableO;
    }

    public String getTableP() {
        return tableP;
    }

    public void setTableP(String tableP) {
        this.tableP = tableP;
    }

    public String getTableQ() {
        return tableQ;
    }

    public void setTableQ(String tableQ) {
        this.tableQ = tableQ;
    }

    public String getTableR() {
        return tableR;
    }

    public void setTableR(String tableR) {
        this.tableR = tableR;
    }

    public String getTableS() {
        return tableS;
    }

    public void setTableS(String tableS) {
        this.tableS = tableS;
    }

    public String getTableT() {
        return tableT;
    }

    public void setTableT(String tableT) {
        this.tableT = tableT;
    }

    public String getTableU() {
        return tableU;
    }

    public void setTableU(String tableU) {
        this.tableU = tableU;
    }

    public String getTableV() {
        return tableV;
    }

    public void setTableV(String tableV) {
        this.tableV = tableV;
    }

    public String getTableW() {
        return tableW;
    }

    public void setTableW(String tableW) {
        this.tableW = tableW;
    }

    public String getTableX() {
        return tableX;
    }

    public void setTableX(String tableX) {
        this.tableX = tableX;
    }

    public String getTableY() {
        return tableY;
    }

    public void setTableY(String tableY) {
        this.tableY = tableY;
    }

    public String getTableZ() {
        return tableZ;
    }

    public void setTableZ(String tableZ) {
        this.tableZ = tableZ;
    }
}
